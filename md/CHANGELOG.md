...
- v1.32 – ControlAltAI: Integrated ControlAltAI utility nodes under `nodes/controlaltai/` (upstream [gseth/ControlAltAI-Nodes](https://github.com/gseth/ControlAltAI-Nodes), MIT). The maintainer’s **Python 3.13–compatible fork** of those nodes is included under `nodes/controlaltai/` in this pack (same node set), **to reduce the maintainer’s own maintenance burden** (one fewer separate custom-node repository to track). Pack registration in root `__init__.py`; README ControlAltAI section (after Florence-2) with node details in [nodes/controlaltai/controlalttai.md](nodes/controlaltai/controlalttai.md) only; frontend helper `js/integer_settings_advanced.js` for Integer Settings Advanced.
