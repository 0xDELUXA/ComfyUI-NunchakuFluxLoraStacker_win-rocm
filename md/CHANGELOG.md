...
- v1.32 – ControlAltAI: Integrated ControlAltAI utility nodes under `nodes/controlaltai/` (upstream lineage [gseth/ControlAltAI-Nodes](https://github.com/gseth/ControlAltAI-Nodes), MIT). **Integration rationale:** these nodes were first maintained in a **separate Python 3.13–compatible fork repository**; they are **merged into this pack** so users manage **one fewer custom-node repository** (clone, pin, and update) while keeping the same node set. Pack registration in root `__init__.py`; README ControlAltAI section (after Florence-2) with node details in [nodes/controlaltai/controlalttai.md](nodes/controlaltai/controlalttai.md) only; frontend helper `js/integer_settings_advanced.js` for Integer Settings Advanced.
